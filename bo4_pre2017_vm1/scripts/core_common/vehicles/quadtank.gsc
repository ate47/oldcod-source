...
function state_combat_update(params) {
    ...
    if (isdefined(0) && isalive(self) && !function_fcd2c4ce() && 0) {
...
function trophy_destroyed() {
    if (self.var_de9eba31 >= 4 || !(isdefined(0) && 0)) {
        return true;
    }
    return false;
}
...
function function_5d17667f() {
    ...
    if (!isdefined(driver) && self vehicle_ai::get_current_state() != "off" && self vehicle_ai::get_next_state() !== "off" && !(isdefined(0) && 0)) {
        self notify(#"pain", {#var_e3ad0f90:1});
    }
    target_set(self, (0, 0, 60));
    self hidepart("tag_defense_active");
    ...
    if (trophy_destroyed()) {
        ...
        level notify(#"hash_27175bcd", {#entity:self});
    ...
    level notify(#"hash_6530962c", {#entity:self});
    self playsound("wpn_trophy_disable");
...
function function_ce00e5c0() {
    ...
    level notify(#"hash_f015cdf7", {#entity:self});
}
...
function function_bcd490eb(var_dcf15942) {
    ...
    self endon(#"change_state");
    self.turretrotscale = 0.3;
    while (!isdefined(self.enemy) || var_dcf15942 || !self cansee(self.enemy)) {
    ...
    while (true) {
        ...
        if (isdefined(self.enemy) && vehicle_ai::iscooldownready("javelin_rocket_launcher", 0.5)) {
            if (isvehicle(self.enemy) || distance2dsquared(self.origin, self.enemy.origin) >= 800 * 800) {
                var_447f1b87 = !self seerecently(self.enemy, 3) || randomint(100) < 3;
            }
        }
    ...
    while (self.allow_movement) {
        if (self.var_872042b7 !== 1) {
            ...
            if (self.goalforced) {
                ...
                var_7f0e7e1 = turret::get_target(1);
                if (!isdefined(var_7f0e7e1)) {
                    var_7f0e7e1 = self.enemy;
                }
                var_2df3624a = turret::get_target(2);
                if (!isdefined(var_2df3624a)) {
                    var_2df3624a = self.enemy;
                }
                goalarray = tacticalquery("quadtank_combat", self, self, self.enemy, var_44367de6, var_7f0e7e1, var_2df3624a);
                if (goalarray.size > 0) {
...
function function_17b8332e() {
    self endon(#"death");
    self notify(#"hash_17b8332e");
    self endon(#"hash_17b8332e");
    /#
...
function function_efc2d52f(einflictor, eattacker, idamage, idflags, smeansofdeath, weapon, vpoint, vdir, shitloc, vdamageorigin, psoffsettime, damagefromunderneath, modelindex, partname, vsurfacenormal) {
    ...
    if (isdefined(0) && vehicle_ai::entityisarchetype(eattacker, "raps") && !function_fcd2c4ce() && 0) {
        self.var_ef5ed6ae = 0;
        self.var_de9eba31 = 3;
        self thread function_5d17667f();
    ...
    if (partname == "tag_target_lower" || partname == "tag_target_upper" || partname == "tag_defense_active" || partname == "tag_body_animate") {
        if (isdefined(eattacker) && isplayer(eattacker) && eattacker.team != self.team) {
            ...
            if (isdefined(0) && !function_fcd2c4ce() && 0) {
    ...
    if (smeansofdeath != "MOD_UNKNOWN" && !vehicle_ai::entityisarchetype(eattacker, "quadtank")) {
        if (isdefined(0) && 0 && self.var_9d0e9d5e + idamage > self.settings.var_f99b9c5 && self.var_de9eba31 < 4 && !isdefined(driver)) {

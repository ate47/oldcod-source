...
function custom_class_update( localclientnum )
{
    ...
    if ( isdefined( weapon_full_name ) )
    {
        ...
        if ( level.last_weapon_name != weapon_full_name )
        {
            ...
            update_weapon_script_model( localclientnum, weapon_full_name, undefined, is_item_unlocked );
        }
...
function handle_cac_customization_weaponoption( localclientnum )
{
    ...
    while ( true )
    {
        level waittill( #"cam_customization_wo", weapon_option, weapon_option_new_index, is_item_locked );
...
function handle_cac_customization_closed( localclientnum )
{
    ...
    {
        level.weapon_clientscript_cac_model[ level.loadout_slot_name ] setweaponrenderoptions( get_camo_index(), get_reticle_index(), get_show_payer_tag(), get_show_emblem(), get_show_paintshop() );

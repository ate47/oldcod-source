    ...
    for ( n_player_index = 0; n_player_index < 4 ; n_player_index++ )
    {
        ...
        for ( n_coll = 0; n_coll <= 8 ; n_coll++ )
        {
            ...
            if ( isdefined( item ) )
            {
                ...
                level.rooms[ n_player_index ].a_coll[ n_coll ] = var_e44d919a;
            }
        }
    }
    ...
    while ( true )
    {
        ...
        if ( slotname == "custom" || slotname == "custom_removeattach" )
        {
            ...
            if ( weaponname == "ar_standard" )
            {
                ...
                for ( i = 1; i < weaponandattachments.size ; i++ )
                {
                    ...
                    if ( weaponandattachments[ i ] == "extbarrel" )
                    {
                        extbarrel = 1;
                    }
                }
        ...
        if ( slotname == "attachment" )
        {
            if ( type == "primary" && isdefined( level.primaryweapon ) && level.primaryweapon == "ar_standard" )
            {
                ...
                if ( weaponorattachment == "reflex" )
                {
                    weaponorattachment = "optics";
                }
                
                camera_ent thread transition_extracam( "ui_cam_cac_attachments_ar_arak", 0.3, 400, "cam_" + weaponorattachment );
            }
        }
    }
}
    ...
    while ( true )
    {
        ...
        if ( var_64bc1c63 )
        {
            if ( event === "clear_preview_collectible" )
            {
                ...
                {
                    collectibleslot = level.rooms[ var_a7f04717 ].a_coll[ collectibleindex ];
...
function function_d9b15719( localclientnum, menu_name )
{
    ...
    for ( n_coll = 0; n_coll <= 8 ; n_coll++ )
    {
        ...
        level.rooms[ var_a7f04717 ].a_coll[ n_coll ].var_9e6f06a8 = undefined;
    }
}
...
{
    enablefrontendstreamingoverlay( localclientnum, 1 );
    character_customization::update_show_helmets( localclientnum, level.liveccdata, 0 );
}
...
{
    character_customization::update_show_helmets( localclientnum, level.liveccdata, 1 );
    enablefrontendstreamingoverlay( localclientnum, 0 );
}

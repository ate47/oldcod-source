...
// Params 0, eflags: 0x0
// Checksum 0x2ca13915, Offset: 0x100
// Size: 0x6fc
function callback_playerdisconnect() {
    ...
    if (!gamestate::is_game_over() && !level.gameended) {
        ...
        self player_record::function_9c92813d(#"disconnect");
    }
    ...
    if (isdefined(self.score) && isdefined(self.pers[#"team"])) {
        /#
            print("<dev string:x30>" + self.pers[#"team"] + "<dev string:x3d>" + self.score);
        #/
        level.dropteam += 1;
    }
    ...
    for (entry = 0; entry < level.players.size; entry++) {
        ...
        if (level.players[entry].pers[#"nemesis_name"] == self.name) {
            level.players[entry] function_7ceb5996();
        }
    }
    if (level.gameended) {
...
// Namespace player/player_disconnect
// Params 0, eflags: 0x4
// Checksum 0xabcf824b, Offset: 0x808
// Size: 0x2c6
function private function_7ceb5996() {
    ...
    assert(isdefined(nemesisname), "<dev string:x3f>" + self.name);
    assert(isstring(nemesisname), "<dev string:x60>" + nemesisname + "<dev string:x69>" + self.name);
